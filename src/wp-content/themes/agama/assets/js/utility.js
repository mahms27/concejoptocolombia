(function($) {
	 ...
	 setTimeout(function(){
	 	...
	 	$(".tribe-events-ical tribe-events-button").text("+ Exportar Eventos");
	 }, 0);
  	...
  	$win.scroll(function () {
	 	...
	 	if ($win.scrollTop() <= $pos){
	 		$("#container div.select").css('top','225px');
	   		$(".bg_img").css('top','197px');
	   		$(".bg_img").css('height','160%');
	   		
	 	}
	 	else {
	     	$("#container div.select").css('top','128px');
	     	//$(".bg_img").css('top','197');
	 	}
	 	if ($win.scrollTop() == 0){
	 		$(".bg_img").css('top','321px');
	 	}
